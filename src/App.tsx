...
const levelConfig = (level: number) => {
  ...
  const targetScore = Math.min(5000, 300 + biomeIndex * 400 + ((level - 1) % 10) * 100);
  const totalMoves = Math.min(400, 20 + biomeIndex * 15 + ((level - 1) % 10) * 2);

  return {
    level,
    targetScore,
    moves: totalMoves,
    world: biomeIndex === 0 ? 'Starlight Meadows' : biomeIndex === 1 ? 'Crystal Valley' : biomeIndex === 2 ? 'Twilight Grove' : 'Celestial Peaks',
    title: `Level ${level}`,
