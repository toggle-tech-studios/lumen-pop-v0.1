...
<span className="font-label-counter text-body-sm text-primary-fixed ml-1">1,240</span>
</div>
...
<script>
  // Simple playful click response for Level 24 and Play Launcher
  const heroNode = document.getElementById('hero-current-node');
  const playBtn = document.getElementById('play-level-btn');

  function triggerLevelLaunch() {
    if (playBtn) {
      playBtn.classList.add('scale-95');
      setTimeout(() => playBtn.classList.remove('scale-95'), 180);
    }
  }

  if (heroNode) {
    heroNode.addEventListener('click', triggerLevelLaunch);
  }
  if (playBtn) {
    playBtn.addEventListener('click', triggerLevelLaunch);
  }
